from clearml_test_utils import (
    ...
    load_clearml_template_spec_module,
)
...
def test_clearml_launch_targets_use_infer_stage_and_training_pipeline_drafts():
    template_spec = load_clearml_template_spec_module()
    assert [name for name, _, _ in template_spec.TEMPLATES] == [
        "tabular_infer_template",
        "tabular_stage_template",
        "tabular_train_pipeline_template",
    ]
    assert [name for name, _, _ in template_spec.TASK_TEMPLATES] == [
        "tabular_infer_template",
        "tabular_stage_template",
    ]
    assert [name for name, _, _ in template_spec.PIPELINE_TEMPLATES] == [
        "tabular_train_pipeline_template",
    ]
    assert template_spec.PIPELINE_TEMPLATES[0][1] == "config/tasks/tabular_pipeline.yaml"
    assert template_spec.entry_point("tabular_stage_template") == "clearml/app.py"
    assert template_spec.entry_point("tabular_train_pipeline_template") == "clearml/pipelines.py"
    assert template_spec.template_tags("tabular_train_pipeline_template") == [
        "domain:tabular",
        "run_type:template",
        "user_facing:true",
    ]
    assert template_spec.template_tags("tabular_stage_template") == [
        "domain:tabular",
        "run_type:template",
        "internal:true",
    ]
...
def test_clearml_infer_template_uses_remote_dataset_defaults():
    ...
    params = template_spec.task_runtime_params("tabular_infer_template", cfg)
...
def test_clearml_template_metadata_replaces_stale_role_tags():
    ...
    infer = FakeTask()
    template_spec.apply_task_metadata(infer, "tabular_infer_template", spec)
