...
|---|---|---|---|---|---|---|---|---|---|---|
| S01 | confirmed unused code removal | repository-wide Python code | Dedicated unused-code tools are not installed, so live unused code cannot be safely confirmed yet. | Run `vulture` and targeted `rg` / import checks before deleting anything. | No, not until confirmed by tools and tests. | Medium | `uv run python -m pytest`, import smoke tests, targeted entrypoint checks | needs_confirmation | pending | Do not treat `docs/review/source` matches as deletion candidates. |
| S02 | stale compatibility wrapper removal | `clearml/adapter.py`, `clearml/pipelines.py`, `clearml/templates.py`, `pkgs/tabular/src/ml_platform_tabular/{plots.py,infer.py,pipeline.py}` | Deprecated UI-named wrappers and facade modules remain after review-response migrations. | Migrate internal callers first, confirm target/external imports, then remove wrappers in small commits. | Only after import and ClearML template compatibility confirmation. | High | full pytest, `tests/test_clearml_mapping.py`, tabular characterization tests, ClearML dry-run | needs_confirmation | pending | Keep ClearML parameter keys stable. |
| S03 | excessive contract/spec simplification | `pkgs/core/src/ml_platform_core/contracts.py`, `runtime_types.py`, `pkgs/tabular/src/ml_platform_tabular/manifest.py`, `policy.py` | Some contracts and Protocols currently have one domain or no concrete implementation. | Decide which boundaries are real extension points; inline or reduce single-use abstractions. | No, not before R18 manual verification and target-port review. | Medium | `tests/test_runtime_manifest.py`, fake renderer tests, full pytest | needs_confirmation | pending | `RuntimeAdapter` and `TaskRunner` are the first candidates to confirm. |
| S04 | diagnostics and error handling simplification | `clearml/adapter.py`, `clearml/reports.py`, `clearml/templates.py`, `clearml/pipelines.py` | Metadata/tag handling, dry-run output, reporting fallback, and figure-building logic are repeated. | Consolidate only behavior-preserving helpers that reduce duplicate diagnostics and error wording. | Yes, for duplicate private helpers after tests cover behavior. | Medium | ClearML mapping tests, report behavior tests, compileall, pytest | todo | pending | CLI `print()` output may remain if it is operator-facing. |
...
3. S02: remove wrappers only after S09 and target-import confirmation.
