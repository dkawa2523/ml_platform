...
from ml_platform_tabular.manifest import TABULAR_STAGE_INPUT_PARAMETERS, get_tabular_manifest
...
_STAGE_INPUT_KEYS = {parameter.name for parameter in TABULAR_STAGE_INPUT_PARAMETERS}
_CONFIG_PATH_EXCEPTIONS = {
    "Run/task": ("task",),
    "Model/model_params_by_name": ("model", "params"),
    "Model/params": ("model", "params"),
    "Model/evaluation_metrics": ("metrics", "names"),
    "Model/ensemble_enabled": ("model", "ensemble", "enabled"),
    "Model/ensemble_methods": ("model", "ensemble", "methods"),
    "Model/ensemble_method": ("model", "ensemble", "method"),
    "Model/ensemble_top_k": ("model", "ensemble", "top_k"),
}
_SECTION_TO_CONFIG = {
    "Input": "data",
    "Split": "split",
...
def _unique_specs(groups: tuple[tuple[ParameterSpec, ...], ...]) -> tuple[ParameterSpec, ...]:
    ...
    for group in groups:
        for spec in group:
            ...
            elif _transport_signature(current) != _transport_signature(spec):
                raise ValueError(f"Conflicting runtime ParameterSpec for {spec.name!r}.")
    return tuple(specs.values())


def _transport_signature(spec: ParameterSpec) -> tuple[object, object, object]:
    return (spec.value_type, spec.default, spec.choices)


def _binding(spec: ParameterSpec) -> RuntimeParamBinding:
    return RuntimeParamBinding(spec.name, _binding_value_type(spec), _config_path(spec.name))
...
def _config_path(key: str) -> tuple[str, ...]:
    if key in _STAGE_INPUT_KEYS:
        return ("stage_inputs", key.split("/", 1)[1])
    if key in _CONFIG_PATH_EXCEPTIONS:
        return _CONFIG_PATH_EXCEPTIONS[key]
    section, leaf = key.split("/", 1)
    config_section = _SECTION_TO_CONFIG.get(section)
    return () if config_section is None else (config_section, leaf)
