...
from .metadata import load_model_info, model_info_path
from .prediction_frame import PREDICTION_SCHEMA_VERSION, model_artifact_id
from .prediction_writer import write_predictions
from .resolver import model_artifact_path, model_selector, model_source_type
from .schema import check_inference_schema, write_schema_check
...
def run_infer(cfg: dict[str, Any]) -> RunResult:
    ...
    features, schema_summary = check_inference_schema(
        cfg,
        df,
        estimator=estimator,
        model_info=model_info,
    )
    schema_check_json_path = write_schema_check(schema_summary, run_dir)
    ...
    artifact_id = model_artifact_id(model_path)
...
def _load_model_context(
    ...
    model_path = model_artifact_path(cfg, output_dir)
    model_info = load_model_info(cfg, model_path)
    validate_model_artifact(
        model_path,
        model_info,
        require_integrity=model_source_type(cfg) == "task_id",
    )
...
def _inference_artifacts(
    ...
    info_path = model_info_path(cfg, model_path)
    if info_path:
...
def _manifest_extra(
    ...
    *,
    ...
    return {
        ...
        "source_type": model_source_type(cfg),
        "source_task_id": model_cfg.get("source_task_id"),
        "model_selector": model_selector(cfg),
