...
from .metrics import (
    ...
    write_metrics_by_model_bar_artifact,
...
def _build_ensemble(
    ...
    plots = write_regression_plot_artifacts(preprocess["y_valid"], y_pred, stage_dir, prefix="ensemble")
    model_path = dump_joblib(estimator, stage_dir / "model.joblib")
    model_info_path = write_model_info(
        stage_dir / "model_info.json",
        feature_columns=preprocess["feature_columns"],
...
def _write_evaluation_predictions(best: dict[str, Any], stage_dir: Path) -> tuple[Path | None, dict[str, Path]]:
    ...
    if {"actual", "prediction"} <= set(frame.columns):
        plots = write_regression_plot_artifacts(frame["actual"], frame["prediction"], stage_dir, prefix="evaluation")
    else:
        plots = {}
...
def _evaluate_models(
    ...
    model_refs_payload = {
        ...
        "ensemble": _model_ref_payload(ensemble_result) if ensemble_result is not None else None,
    }
    ...
    metrics_by_model_bar_path = write_metrics_by_model_bar_artifact(metrics_by_model, stage_dir)
...
def _run_training_pipeline(cfg: dict[str, Any]) -> RunResult:
    ...
    tables: dict[str, Path] = {
        **preprocess["tables"],
        "leaderboard": evaluation["tables"]["leaderboard"],
        **({"evaluation_predictions": evaluation["tables"]["evaluation_predictions"]} if "evaluation_predictions" in evaluation["tables"] else {}),
    }
    ...
    if ensemble_result is not None:
        ...
        for plot_name, plot_path in ensemble_result.get("plots", {}).items():
            plots[f"{plot_name}_ensemble"] = plot_path
